# Parité avec Branlly Windows

La version PowerShell/WPF reste la spécification fonctionnelle. Une fonction n’est considérée portée que si elle est disponible sur une plateforme réelle, testée et présentée dans le HUD.

| Domaine | Fonction | État Branlly Next |
|---|---|---|
| Compagnon | 32 frames transparentes | ✅ |
| Compagnon | déplacement manuel | ✅ |
| Compagnon | humeur et énergie | ✅ cœur, affichage partiel |
| Compagnon | suivi du curseur | ⬜ |
| Compagnon | sons et réactions | ⬜ |
| Interface | HUD radial | 🟡 moteur initial |
| Interface | sous-menus complets | ⬜ |
| IA | Ollama `qwen2.5:3b` | ✅ |
| IA | streaming et annulation | ✅ |
| IA | personnalité historique | ✅ prompt local |
| IA | mémoire persistante sur disque | ⬜ |
| Applications | raccourcis configurables | ⬜ |
| Applications | fenêtres ouvertes | ⬜ |
| Applications | activation/fermeture | ⬜ |
| Système | état Wi-Fi | ⬜ |
| Système | Bluetooth | ⬜ |
| Système | périphériques | ⬜ |
| Système | nettoyage | ⬜ |
| Recherche | recherche Wikipédia | ⬜ |
| Jeux | Metro Rush | ✅ |
| Jeux | BlockCraft Lite | ✅ |
| Jeux | Pong | ⬜ |
| Jeux | Desktop Whip | ⬜ |
| Distribution | AppImage et DEB | ✅ |
| Distribution | MSI et EXE | ✅ |

Légende : ✅ porté ; 🟡 partiel ; ⬜ à porter.
